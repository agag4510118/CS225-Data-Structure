...
class BinaryTree
{
	...
	// Helper function for isoerdered
	bool isOrdered(Node * node,bool t) const;
	//Helper get right node of left subtree
	Node * rightofleft(Node * node) const;
	//Helper get left node of right subtree
        Node * leftofright(Node * node) const;

	//Helper for printpath
	void printPaths(Node *node,vector<T> path) const;
	void printvector(vector<T> path) const;
	
	//sumdistance
        int sumDistances(Node * node, int depth) const;
