...
LogfileParser::LogfileParser( const string & fname ) : whenVisitedTable( 256 ) {
	...
	// Unique URl
string tmp = ll.customer + ll.url ;
//pageVisitedTable.insert(tmp,true);
	...
	// Hashtable
	if(whenVisitedTable.keyExists(tmp))
	{
		if(whenVisitedTable[tmp] < ll.date){
			whenVisitedTable[tmp] = ll.date;
		}
		
	}	
	else
		whenVisitedTable.insert(tmp,ll.date);
...
bool LogfileParser::hasVisited( const string & customer, const string & url ) const {
    /**
     * @todo Implement this function.make lfparse
     */
    bool result = false;
    string tmp = customer + url;
    result = whenVisitedTable.keyExists(tmp);
...
string tmp = customer + url;
	...
	return time_t();
        ...
        return whenVisitedTable.find(tmp); // replaceme
