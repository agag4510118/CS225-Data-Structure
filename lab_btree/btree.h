...
size_t inserthelp(const std::vector< T >& elements, const C& val, size_t  start ,size_t  end)
{
	size_t  middle = (start + end)/2;
	size_t  d = (end - start)/2;
	...
	if (elements[middle] == val)
		return middle;
	if(d>=1){
	if (elements[middle]>val && elements[middle-1]<val )
		return middle;
	if(elements[middle]<val && elements[middle+1]>val)
		return middle +1;
	if(elements[middle-1] == val)
		return middle-1;
	if (elements[middle+1] == val)
		return middle +1;
	}
	...
	if(val>elements[middle])
		return inserthelp(elements,val,middle+1,end);
	else
		return inserthelp(elements,val,start,middle-1);
	//if(val < elements[middle])
		//inserthelp(elements,val,start,middle);	
...
/*
	...
	auto up = upper_bound(start,end,val);
...
//size_t result = inserthelp(elements,val,start,end);
 return result;
