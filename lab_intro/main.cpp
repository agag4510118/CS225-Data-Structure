...
PNG blendImages(PNG firstImage, PNG secondImage)
{
 	...
 	for (size_t yi = 0; yi < firstImage.height(); yi++)
    {
        ...
        {
		firstImage(xi,yi)->red = ((int)firstImage(xi,yi)->red + (int)secondImage(xi,yi)->red)/2;
		firstImage(xi,yi)->green = ((int)firstImage(xi,yi)->green + (int)secondImage(xi,yi)->green)/2;
 		firstImage(xi,yi)->blue = ((int)firstImage(xi,yi)->blue + (int)secondImage(xi,yi)->blue)/2;	
	}
	}
