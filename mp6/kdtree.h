...
class KDTree
{
    ...
    private:
         ...
         */
        void kdtreehelper(vector< Point<Dim> > & newPoints,int left, int right, int d);
        int partition(vector< Point<Dim> > & p, int left, int right, int indexn, int d);
        Point<Dim> select(vector< Point<Dim> > & p, int left, int right, int n, int d);
    
        int distance(const Point<Dim> first, const Point<Dim> second) const;
       Point<Dim> findhelper(const Point<Dim> & query, int left, int right, int d) const;
